...
def pad_molecule_tensor(molecule_tensor, final_size):
    ...
    and pads it to final_size * final_size * attribute_num
    using zeros
    """
...
def get_molecule_tensor(molecule, 
    ...
    for i, atom in enumerate(non_H_atoms):
        molecule_tensor[i, i, :] = np.concatenate((atom_attributes_dict[atom],
                                                   np.zeros_like(bond_attributes_dict.values()[0])))
